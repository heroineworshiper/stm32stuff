...
#define PET_WATCHDOG IWDG->KR = KR_KEY_RELOAD;
...
#define READ_FLOAT8(buffer, offset) \
({ \
	offset++; \
	((float)(buffer[offset - 1]) / FRACTION); \
})

#define READ_FLOAT16(buffer, offset) \
({ \
	offset += 2; \
	(((float)((int16_t)(buffer[offset - 2] | (buffer[offset - 1] << 8)))) / FRACTION); \
})
...
#define WRITE_FLOAT16(buffer, offset, value) \
	(buffer)[(offset)++] = ((int)((value) * FRACTION)) & 0xff; \
	(buffer)[(offset)++] = (((int)((value) * FRACTION)) >> 8) & 0xff;

void write_float32(unsigned char *buffer, float x);
#define WRITE_FLOAT32(buffer, offset, value) \
	write_float32(buffer + offset, value); \
	offset += 4;
