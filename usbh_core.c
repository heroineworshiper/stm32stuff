...
//  USB_OTG_BSP_ConfigVBUS(0);  
  ...
  pdev->host.port_cb->Disconnect = USBH_Disconnect;
  pdev->host.port_cb->Connect = USBH_Connect;
  pdev->host.port_cb->ConnStatus = 0;   
  pdev->host.port_cb->DisconnStatus = 0; 
  pdev->host.port_cb->ConnHandled = 0; 
...
//TRACE
//print_text("phost->gState=");
//print_number(phost->gState);
//print_lf();
  switch (phost->gState)
  {
...
/*
 * TRACE
 * print_text("EnumState=");
 * print_number(phost->EnumState);
 * print_lf();
 */
...
/*
 * TRACE
 * print_text("Control.state=");
 * print_number(phost->Control.state);
 * print_lf();
 */
...
/*
 * print_text("URB_Status=");
 * print_number(URB_Status);
 * print_lf();
 */
   	...
   	if(URB_Status == URB_DONE)
    {
      ...
      if (phost->Control.setup.b.wLength != 0 )
      {        
...
/*
 * TRACE
 * print_text("phost->Control.length=");
 * print_number(phost->Control.length);
 * print_lf();
 */
...
// HACK
// Dying in CTRL_STATUS_IN_WAIT if phost->Control.setup.b.wLength != 0
// Must read back the write value to get the write to succeed.
//     phost->Control.state = CTRL_STATUS_IN;
       phost->gState =   phost->gStateBkp;
   }
...
/*
 * TRACE
 * print_text("hc_num_in=");
 * print_hex2(phost->Control.hc_num_in);
 * print_text(" hc_num_out=");
 * print_hex2(phost->Control.hc_num_out);
 * print_lf();
 */
