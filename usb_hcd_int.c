...
/*
 * print_text("USB_OTG_USBH_handle_hc_n_Out_ISR hcint.d32=");
 * print_lf();
 * flush_uart();
 */
//print_hex(hcint.d32);
...
// Send next fragment if a multi-fragment URB
	  if(pdev->host.tx_remaining[num] > 0)
	  {
	  		int fragment = pdev->host.tx_remaining[num];
			if(fragment > 64) fragment = 64;
			if(fragment % 4) fragment += 4 - (fragment % 4);

			USBH_BulkSendData (pdev,
				pdev->host.tx_ptr[num], 
				fragment, 
				num);
			pdev->host.tx_ptr[num] += fragment;
			pdev->host.tx_remaining[num] -= fragment;
//print_text("Z");
	  }
    }
...
//print_text("USB_OTG_USBH_handle_hc_n_In_ISR hcint.d32=");
/*
 * print_hex(hcint.d32);
 * print_lf();
 * flush_uart();
...
#ifdef ALWAYS_RX
// Swap buffer
	if(pdev->rx_requested)
	{



print_number_nospace(pdev->interrupt_buffer);
		if(pdev->host.URB_State[num] == URB_DONE)
		{
			pdev->buffer_filled[pdev->interrupt_buffer] = 1;
			pdev->buffer_size[pdev->interrupt_buffer] = pdev->host.XferCnt[num];
			if(!pdev->buffer_filled[!pdev->interrupt_buffer])
			{
				pdev->interrupt_buffer = !pdev->interrupt_buffer;
			}
		}

// but this doesn't crash the USB
//mdelay(10);
// Start next receive immediately
		pdev->buffer_filled[pdev->interrupt_buffer] = 0;
		pdev->buffer_size[pdev->interrupt_buffer] = 0;
		USBH_BulkReceiveData( pdev, 
			pdev->rx_buffers[pdev->interrupt_buffer], 
			IN_BUFFER_SIZE, 
			num);
	}
#endif
...
/*
 * TRACE
 * print_text("grxsts.b.bcnt=");
 * print_number(grxsts.b.bcnt);
 * print_lf();
 */
